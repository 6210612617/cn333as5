...
@Composable
fun ImportantPersonsScreen(viewModel: MainViewModel) {
...
private fun ImportantPersonsList(
    ...
    val importantPersons = contacts.filter{ it.isImportantPersons }
    LazyColumn {
...
private fun ImportantPersonsPreview() {
    ImportantPersonsList(
        contacts = listOf(
            ContactModel(1, "Z", "Content 1", null,false),
            ContactModel(2, "M", "Content 2", false, true),
            ContactModel(3, "L", "Content 3", true,true)
        ).sortedBy { it.name },
        onContactCheckedChange = {},
        onContactClick = {}
    )
